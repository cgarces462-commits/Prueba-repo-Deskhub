...
echo "======================================"
echo " Facturación EPM - arranque del proyecto"
echo "======================================"
...
if mysqladmin -h 127.0.0.1 -P "$PUERTO_DB" -u root ping >/dev/null 2>&1; then
  ...
  if mysqladmin -h 127.0.0.1 -P "$PUERTO_DB" -u root ping >/dev/null 2>&1; then
    mysql -h 127.0.0.1 -P "$PUERTO_DB" -u root \
      -e "CREATE DATABASE IF NOT EXISTS facturacion_medellin" >/dev/null 2>&1 || true
    echo "OK  MySQL listo"
  else
...
# 4) Seed (roles + super_admin + datos)
...
echo
echo "======================================"
echo " Facturación EPM disponible en:"
