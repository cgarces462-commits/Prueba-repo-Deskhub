...
async function listar(usuarioAutenticado) {
  ...
  if ([ROLES.CLIENTE, ROLES.INVITADO].includes(usuarioAutenticado.role)) {
    return reservaRepository.findByUser(usuarioAutenticado.id);
  }
...
async function crear(data, usuarioAutenticado) {
  ...
  const traslapes = await reservaRepository.findOverlapping(spaceId, inicio, fin);
...
async function cambiarEstado(id, estado) {
  ...
  const actualizada = await reservaRepository.updateById(id, { estado });
